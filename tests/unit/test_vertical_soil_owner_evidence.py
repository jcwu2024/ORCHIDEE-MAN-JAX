...
OUTPUT = ROOT / "outputs/reference_mode/micro_oracles/vertical_soil_owner"


def test_vertical_soil_owner_evidence_is_complete() -> None:
    ...
    assert coverage["covered_arm_count"] == coverage["required_arm_count"] == 8
    assert owners["complete"] is True
    assert len(owners["records"]) == 1
    assert owners["records"][0]["owner_region_id"] == "pft14-owner-contract-81f33a7a6b72"
