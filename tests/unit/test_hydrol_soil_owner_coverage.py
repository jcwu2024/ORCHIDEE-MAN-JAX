...
def test_hydrol_soil_owner_evidence_is_complete() -> None:
    output = (
        ROOT
        / "outputs/reference_mode/micro_oracles"
    ...
    assert coverage["required_arm_count"] == coverage["covered_arm_count"] == 109
    assert owners["complete"]
    assert len(owners["records"]) == 13
