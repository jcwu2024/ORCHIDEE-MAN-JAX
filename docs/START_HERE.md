...
is rejected. The formal `pft14-daily-teacher-v6` dataset is complete and
accepted across 669 landpoints and 1961-2010. Its storage and training assets
are frozen; do not regenerate it or join historical v5 rows. The next
operation is admission of the first neural process-head candidate through the
accepted Gate E1 boundary. Paid training remains deferred until that candidate
has a predeclared mechanism, matched control, local smoke, and bounded rollout
gates.
...
drift. Gate E2 data production is complete. The active technical task is the
first formal neural process-head candidate described in `NEXT_STEPS.md`;
long-rollout, Gate D2, and product gates remain dependency-ordered later work.
Before choosing or inheriting
