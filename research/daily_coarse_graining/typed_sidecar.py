"""Lossless typed supplemental labels for daily Teacher data products.

The reader preserves the historical immutable-parent format and also supports
the coherent format where base and typed shards share one release manifest and
are generated by the same continuous Teacher transition.
"""
...
def load_typed_sidecar_contract(
    ...
    if schema_version == COHERENT_CONTRACT_SCHEMA_VERSION:
        ...
        label_identity = raw.get("label_schema", {})
        label_path = ROOT / str(label_identity.get("path", ""))
        if (
            not label_path.is_file()
            or _sha256_file(label_path) != label_identity.get("sha256")
        ):
            raise ValueError("typed-supplement label-schema file hash mismatch")
        label_contract = load_typed_sidecar_contract(label_path)
        if label_contract.sha256 != label_identity.get("contract_sha256"):
            raise ValueError("typed-supplement label-schema contract drift")
    ...
    return TypedSidecarContract(
        ...
        sha256=actual_hash,
        schema_version=schema_version,
        generation_policy="immutable_parent_teacher_forcing",
...
def compiled_daily_flux_fields(
    labels: Any,
    *,
    contract: TypedSidecarContract,
) -> tuple[Any, ...]:
    """Project one traced daily-label PyTree in frozen contract field order."""

    if labels is None:
        raise ValueError("compiled typed capture requires daily flux labels")
    roots = {
        "water": labels.sechiba.water,
        "energy": labels.sechiba.energy,
        "ok_leak": labels.ok_leak,
    }
    values = []
    for field in contract.fields:
        family, separator, name = field.path.partition(".")
        if not separator or family not in roots or not name:
            raise ValueError(f"invalid compiled typed field path: {field.path}")
        value = getattr(roots[family], name)
        if value.shape[0] != 1:
            raise ValueError(f"compiled typed field has non-singleton point axis: {field.path}")
...
def load_coherent_typed_sidecar_index(
    manifest_path: str | Path,
    *,
    contract_path: str | Path = COHERENT_CONTRACT_PATH,
    verify_hashes: bool = True,
) -> TypedSidecarIndex:
    """Load co-generated base and typed shards from one release manifest."""
    ...
    if release.get("transition_policy") != "single_pass_continuous_teacher":
        raise ValueError("coherent Teacher transition policy drift")
    if release.get("typed_contract_sha256") != contract.sha256:
        raise ValueError("coherent Teacher typed contract drift")

    parent_index = load_dataset_index(manifest_path, verify_hashes=verify_hashes)
    if parent_index.dataset_id != contract.parent_dataset_id:
        raise ValueError("coherent Teacher dataset family mismatch")
    if parent_index.contract_sha256 != contract.parent_contract_sha256:
        raise ValueError("coherent Teacher Markov contract mismatch")
    parent_by_key = {
        (reference.landpoint_id, reference.year): reference
        for reference in parent_index.shards
    }
    ...
    for item in raw.get("shards", ()):
        key = (str(item["landpoint_id"]), int(item["year"]))
        parent = parent_by_key[key]
        path = (root / str(item["typed_shard"])).resolve()
        try:
            path.relative_to(root)
        except ValueError as error:
            raise ValueError(f"typed shard path escapes dataset root: {path}") from error
        if not path.is_file():
            raise FileNotFoundError(path)
        expected_hash = str(item["typed_shard_sha256"])
        if verify_hashes and _sha256_file(path) != expected_hash:
            raise ValueError(f"typed shard hash mismatch for {key[0]}:{key[1]}")
        references.append(
            TypedSidecarShardRef(
                parent=parent,
                path=path,
    ...
    if len(references) != len(parent_by_key):
        raise ValueError("coherent Teacher typed shard inventory is incomplete")
    return TypedSidecarIndex(
        contract=contract,
        parent_index=parent_index,
