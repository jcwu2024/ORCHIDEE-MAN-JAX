#!/bin/bash
# Equal-update on-policy pushforward candidate against canonical_multistep_v1.
#SBATCH -J orcjax-pf-ab
#SBATCH -p gnall
#SBATCH --nodes=1
...
#SBATCH -o /WORK/liwei_work/jcwu/ORCHIDEE-MAN-JAX/runtime/logs/canonical_pushforward_ab_%j.txt
#SBATCH -e /WORK/liwei_work/jcwu/ORCHIDEE-MAN-JAX/runtime/logs/canonical_pushforward_ab_%j.txt
#SBATCH --no-requeue
...
INITIALIZE_CHECKPOINT=${INITIALIZE_CHECKPOINT:?set the accepted one-step checkpoint}
...
case "$OUTPUT_DIR" in
  "$ROOT/runtime/outputs/experiments/"*) ;;
  *) echo "OUTPUT_DIR must stay under runtime/outputs/experiments" >&2; exit 2 ;;
esac
...
env \
  ...
  singularity exec \
    ...
    "$PYTHON" -m scripts.hpc.run_canonical_pushforward_gpu_train \
      ...
      --prefix-curriculum 0:48,1:48,3:48,7:48 \
      --batch-size 4 \
