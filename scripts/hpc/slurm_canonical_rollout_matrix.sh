#!/bin/bash
# One-process four-way neural-rollout evaluation on Explore1000.
#SBATCH -J orcjax-rollout-matrix
#SBATCH -p gnall
#SBATCH --nodes=1
...
#SBATCH -o /WORK/liwei_work/jcwu/ORCHIDEE-MAN-JAX/runtime/logs/canonical_rollout_matrix_%j.txt
#SBATCH -e /WORK/liwei_work/jcwu/ORCHIDEE-MAN-JAX/runtime/logs/canonical_rollout_matrix_%j.txt
#SBATCH --no-requeue
...
CHECKPOINT=${CHECKPOINT:?set CHECKPOINT to the candidate checkpoint}
MATRIX=${MATRIX:?set MATRIX to the versioned rollout matrix}
OUTPUT_DIR=${OUTPUT_DIR:?set OUTPUT_DIR to a new validation directory}
...
env \
  ...
  singularity exec \
    ...
    "$PYTHON" -m scripts.hpc.run_canonical_gpu_rollout_matrix \
      --matrix "$MATRIX" \
      --dataset "$DATASET" \
...
test -f "$OUTPUT_DIR/matrix_report.json"
