#!/bin/bash
# Replay one hash-bound persisted driver series through the current exact scan.
#SBATCH --job-name=orcjax-okleak-replay
...
#SBATCH --output=/WORK/liwei_work/jcwu/ORCHIDEE-MAN-JAX/runtime/logs/ok_leak_replay_%j.txt
#SBATCH --error=/WORK/liwei_work/jcwu/ORCHIDEE-MAN-JAX/runtime/logs/ok_leak_replay_%j.txt
#SBATCH --no-requeue
...
LANDPOINT_ID=${LANDPOINT_ID:?set the capture landpoint}
YEAR=${YEAR:?set the capture year}
DAY_INDEX=${DAY_INDEX:?set the capture day}
DRIVER_NPZ=${DRIVER_NPZ:?set the persisted driver NPZ}
...
case "$DRIVER_NPZ" in
  "$ROOT/runtime/outputs/"*.npz) ;;
  *) echo "DRIVER_NPZ must stay under $ROOT/runtime/outputs" >&2; exit 2 ;;
esac
case "$OUTPUT_DIR" in
