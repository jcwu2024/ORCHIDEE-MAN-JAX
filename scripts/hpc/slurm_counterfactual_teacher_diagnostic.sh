#!/bin/bash
# One-point counterfactual Teacher re-entry diagnostic on Explore1000.
#SBATCH -J orcjax-cf-teacher
#SBATCH -p gnall
#SBATCH --nodes=1
...
#SBATCH -o /WORK/liwei_work/jcwu/ORCHIDEE-MAN-JAX/runtime/logs/counterfactual_teacher_%j.txt
#SBATCH -e /WORK/liwei_work/jcwu/ORCHIDEE-MAN-JAX/runtime/logs/counterfactual_teacher_%j.txt
#SBATCH --no-requeue
...
CHECKPOINT=${CHECKPOINT:?set CHECKPOINT to the accepted v1 multistep checkpoint}
OUTPUT=${OUTPUT:?set OUTPUT to a new diagnostic JSON path}
LANDPOINT_ID=${LANDPOINT_ID:-281.0-095.0}
YEAR=${YEAR:-2004}
START_DAY=${START_DAY:-100}
...
for path in "$DATASET" "$STATISTICS" "$ACCEPTANCE" "$PLAN" "$CHECKPOINT"; do
  case "$path" in
    "$ROOT/runtime/"*) ;;
...
case "$OUTPUT" in
  ...
  *) echo "OUTPUT must stay under $ROOT/runtime/outputs" >&2; exit 2 ;;
esac

test -x "$PYTHON"
test "$(cd "$WORKTREE" && git rev-parse HEAD)" = "$EXPECTED_GIT_HEAD"
test ! -e "$OUTPUT"
mkdir -p "$ROOT/runtime/logs" "$(dirname "$OUTPUT")" \
  "$ROOT/runtime/cache/jax/orcjax_gpu"
...
env \
  ...
  singularity exec \
    ...
    "$PYTHON" -m scripts.hpc.run_counterfactual_teacher_gpu \
      --dataset "$DATASET" \
