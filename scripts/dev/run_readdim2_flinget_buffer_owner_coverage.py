...
from oracle_lane_readdim2_flinget_buffer import (  # noqa: E402
    FAMILY,
    PROCEDURES,
    SOURCE,
    compose,
...
def _execute(executable: Path, build: Path, compiler: Path) -> None:
    ...
    subprocess.run(
        [str(executable), str(build / "normal.csv"), "normal"],
        cwd=build,
        env=environment,
        check=True,
        capture_output=True,
    ...
    negative = subprocess.run(
        [str(executable), str(build / "negative.csv"), "negative"],
        cwd=build,
        env=environment,
        check=False,
        capture_output=True,
    ...
    if negative.returncode == 0 or "ipslerr_p" not in negative.stderr:
        raise RuntimeError("negative NBUFF did not reach the Fortran fatal exit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_extracted_owner_coverage(
            ...
            gcov=arguments.gcov.resolve(),
            execute_harness=_execute,
            condition_terminal_lines={2384: 2385},
        )
