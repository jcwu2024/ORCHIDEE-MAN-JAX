...
from oracle_lane_readdim2_geometry import (  # noqa: E402
    FAMILY,
    PROCEDURES,
    SOURCE,
    compose,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_extracted_owner_coverage(
            ...
            arm_branch_indices={
                f"{SOURCE.relative_to(ROOT).as_posix()}:2309:if:true": 9,
                f"{SOURCE.relative_to(ROOT).as_posix()}:2309:if:false": 10,
                f"{SOURCE.relative_to(ROOT).as_posix()}:2310:if:true": 9,
                f"{SOURCE.relative_to(ROOT).as_posix()}:2310:if:false": 10,
            },
        )
