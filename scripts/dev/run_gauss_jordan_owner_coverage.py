...
from oracle_lane_gauss_jordan_owner import (  # noqa: E402
    FAMILY,
    PROCEDURES,
    SOURCE,
    compose,
...
def execute(exe, build, compiler):
    ...
    result = subprocess.run(
        [str(exe), str(build / "sing.csv"), "singular"],
        cwd=build,
        env=env,
        check=False,
        capture_output=True,
    ...
    if "matrix A is not inversible" not in result.stderr:
        raise RuntimeError("singular case missed source STOP")
...
def run_coverage(output_dir=DEFAULT_OUTPUT_DIR, compiler=DEFAULT_COMPILER, gcov=GCOV):
    return run_extracted_owner_coverage(
        ...
        condition_terminal_lines={232: 230},
    )
