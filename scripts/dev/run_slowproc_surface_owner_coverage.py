...
from oracle_lane_slowproc_surface_owners import (  # noqa: E402
...
def run_coverage(output_dir=DEFAULT_OUTPUT_DIR, compiler=DEFAULT_COMPILER, gcov=GCOV):
    return run_extracted_owner_coverage(
        ...
        run_numerical_oracle=run_oracle, output_dir=output_dir,
        compiler=compiler, gcov=gcov,
        coverage_base_flags=tuple(flag for flag in COMPILE_FLAGS if flag != "-fcheck=all"),
    )


def main(argv=None):
    parser = argparse.ArgumentParser(); parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--compiler", type=Path, default=DEFAULT_COMPILER); parser.add_argument("--gcov", type=Path, default=GCOV)
