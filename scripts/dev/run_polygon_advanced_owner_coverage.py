...
from oracle_lane_polygon_advanced_owners import (  # noqa: E402
    FAMILY,
    PROCEDURES,
    SOURCE,
    compose,
...
def execute(executable: Path, build: Path, compiler: Path) -> None:
    ...
    for mode, message in (
        ("intersection_capacity", "polygones_intersection"),
        ("crossing_capacity", "polygones_crossing"),
    ):
        ...
        if message not in failed.stderr and message not in failed.stdout:
            raise RuntimeError(f"{mode} missed expected fatal contract")
