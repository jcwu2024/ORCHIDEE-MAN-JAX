...
from run_stomate_daily_maintenance_oracle import (  # noqa: E402
    _compose,
    _extracts,
    run_oracle,
)
...
def run_coverage(
    ...
    if comparison["status"] != "passed":
        raise RuntimeError("STOMATE daily/maintenance comparison did not pass")
    ...
    target_arms = [
        ...
        if record.get("fortran_procedure") in PROCEDURES
    ...
    with tempfile.TemporaryDirectory(prefix="orchidee_stomate_daily_gcov_") as temporary:
        ...
        generated_spans = {
            name: (
                locate_generated_span(source_bytes, spans[name].span_bytes),
                int(spans[name].start_line),
                spans[name].span_sha256,
            )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run STOMATE daily/maintenance gcov evidence.")
