...
from oracle_lane_haversine_primitives import (  # noqa: E402
    FAMILY,
    PROCEDURES,
    SOURCE,
    compose,
...
def execute(exe, build, compiler):
    env = compiler_environment(compiler)
    subprocess.run(
        [str(exe), str(build / "out.csv")],
        cwd=build,
        env=env,
        check=True,
        capture_output=True,
    ...
    bad = subprocess.run(
        [str(exe), str(build / "bad.csv"), "diagonal"],
        cwd=build,
        env=env,
        check=False,
        capture_output=True,
    ...
    if "ERROR STOP ipslerr" not in bad.stderr:
        raise RuntimeError("diagonal polygon missed ipslerr")
...
def run_coverage(output_dir=DEFAULT_OUTPUT_DIR, compiler=DEFAULT_COMPILER, gcov=GCOV):
    return run_extracted_owner_coverage(
        ...
        arm_branch_indices={
            "fortran_source/ORCHIDEE/src_global/haversine.f90:783:if:true": 6,
            "fortran_source/ORCHIDEE/src_global/haversine.f90:783:if:false": 7,
        },
    )
